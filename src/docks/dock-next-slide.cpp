...
void NotesDock::updateFromInfo(const ppt::SlideInfo &info, bool connected)
{
	...
	QString text;
	if (!info.nextNotes.empty()) {
		size_t len = os_wcs_to_utf8(info.nextNotes.c_str(), info.nextNotes.length(), nullptr, 0);
		std::string utf8(len, '\0');
		os_wcs_to_utf8(info.nextNotes.c_str(), info.nextNotes.length(), &utf8[0], len + 1);
...
void RegisterDocks(ppt::ComBridge *bridge)
{
	...
	obs_frontend_add_dock_by_id("PPTNextSlideDock", obs_module_text("PPT.NextSlideDock.Title"),
				    new NextSlideDock(bridge));

	obs_frontend_add_dock_by_id("PPTNotesDock", obs_module_text("PPT.NotesDock.Title"), new NotesDock(bridge));
